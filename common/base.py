...
class Base():
    ...
    def find(self, locator):
        ...
        if not isinstance(locator, tuple):
            ...
            try:
                ...
                raise ElementNotFound("定位元素出现超时！查看定位是否正确，别复制粘贴xpath，要先在浏览器调试成功，观察页面是否正常打开")
            return ele

    def finds(self, locator):
        '''复数定位，返回elements对象list[]
        :param locator:
        :return:
        '''
        if not isinstance(locator, tuple):
    ...
    def send(self, locator, text=''):
        ...
        if ele.is_displayed():
            ...
            raise ElementNotVisibleException("元素不可见或者不唯一无法输入，解决办法：定位唯一元素，或先让元素可见，或用js输入")
    ...
    def click(self, locator):
       ...
       if ele.is_displayed():
           ...
           raise ElementNotVisibleException("元素不可见或者不唯一无法点击，解决办法：定位唯一元素，或先让元素可见，或用js点击")
